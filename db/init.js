...
function getDiasGuilherme(f, uid) {
  // Dieta padronizada para toda a semana (~2150 kcal)
  const dia = (dow, nome) => ({ userId: uid, dow, name: nome, foods: [
    // ── Café da Manhã ─────────────────────────────────
    f('Ovos inteiros (3)',                   180, 216, 18.0,  1.2, 15.0, 'cafe_manha', 0),
    f('Aveia em flocos',                      40, 155,  6.8, 26.4,  2.8, 'cafe_manha', 1),
    f('Banana',                              120, 118,  1.6, 31.0,  0.1, 'cafe_manha', 2),
    f('Iogurte natural',                     100,  60,  3.5,  4.7,  2.0, 'cafe_manha', 3),
    // ── Almoço ────────────────────────────────────────
    f('Peito de frango grelhado',            160, 264, 49.6,  0.0,  5.8, 'almoco', 0),
    f('Arroz branco cozido',                 150, 182,  3.8, 40.5,  0.3, 'almoco', 1),
    f('Feijão cozido',                        80,  92,  5.4, 17.2,  0.4, 'almoco', 2),
    f('Salada (alface, tomate, cebola, pimentão)', 150, 30, 1.5, 5.5, 0.2, 'almoco', 3),
    f('Azeite (1 colher chá)',                 5,  44,  0.0,  0.0,  5.0, 'almoco', 4),
    // ── Lanche da Tarde ───────────────────────────────
    f('Pão de forma (2 fatias)',              60, 167,  5.0, 30.0,  3.0, 'cafe_tarde', 0),
    f('Atum enlatado (1 lata)',               85, 100, 22.0,  0.0,  0.8, 'cafe_tarde', 1),
    f('Queijo light',                         30,  72,  7.2,  1.5,  4.2, 'cafe_tarde', 2),
    f('Maçã',                                150,  84,  0.5, 22.5,  0.3, 'cafe_tarde', 3),
    f('Creatina',                              5,   0,  0.0,  0.0,  0.0, 'cafe_tarde', 4),
    // ── Pós-Treino + Jantar ───────────────────────────
    f('Whey protein (1 scoop)',               30, 120, 24.0,  3.0,  2.0, 'janta', 0),
    f('Banana (pós-treino)',                 120, 118,  1.6, 31.0,  0.1, 'janta', 1),
    f('Peito de frango grelhado',            160, 264, 49.6,  0.0,  5.8, 'janta', 2),
    f('Batata cozida / assada',              200, 174,  3.8, 40.0,  0.2, 'janta', 3),
    f('Salada mista',                        100,  20,  1.5,  3.0,  0.3, 'janta', 4),
    f('Queijo fresco batido',                 30,  30,  2.1,  1.8,  1.2, 'janta', 5),
  ]});

  return [
    dia(1, 'Segunda — Treino A'),
    dia(2, 'Terça — Treino B'),
    dia(3, 'Quarta — Treino C'),
    dia(4, 'Quinta — Treino D'),
    dia(5, 'Sexta — Treino E'),
    dia(6, 'Sábado — Descanso'),
    dia(0, 'Domingo — Descanso'),
  ];
}
  ...
  const plans = {
    guixz: [
      { dow: 1, name: 'Treino A — Ombros & Dorsais', exercises: [
        ...
        e('Remada Baixa (pegada neutra)',          3, 11, 2),
        e('Elevação Lateral na Polia',            4, 13, 3),
        e('Desenvolvimento c/ Halteres',          3,  9, 4),
      ]},
      { dow: 2, name: 'Treino B — Peitoral & Tríceps', exercises: [
        e('Supino Inclinado c/ Halteres',         4, 10, 0),
        e('Supino Reto',                          3,  9, 1),
        e('Crossover / Crucifixo na Polia',       3, 13, 2),
        e('Tríceps Corda na Polia',               4, 11, 3),
        e('Tríceps Testa',                        3, 11, 4),
      ]},
      { dow: 3, name: 'Treino C — Pernas & Core', exercises: [
        e('Agachamento Livre / Leg Press',        4,  9, 0),
        e('RDL / Stiff c/ Halteres',              4, 11, 1),
        e('Cadeira Extensora',                    3, 13, 2),
        e('Mesa / Cadeira Flexora',               3, 13, 3),
        e('Panturrilha em Pé',                    4, 15, 4),
        e('Elevação de Pernas (Abdômen)',          3, 20, 5),
      ]},
      { dow: 4, name: 'Treino D — Costas & Bíceps', exercises: [
        e('Puxada Alta (pegada triângulo)',        4, 11, 0),
        e('Remada Serrote c/ Halter',             3, 10, 1),
        e('Crucifixo Inverso / Deltóide Post.',   4, 13, 2),
        e('Rosca Direta',                         3, 11, 3),
        e('Rosca Martelo',                        3, 11, 4),
      ]},
      { dow: 5, name: 'Treino E — Ombro & Tronco', exercises: [
        e('Elevação Lateral (dropset na última)', 5, 13, 0),
        e('Supino Inclinado Articulado',          3, 10, 1),
        e('Puxada Alta Pegada Supinada',          3, 10, 2),
        e('Prancha Isométrica (45-60s)',           3,  1, 3),
      ]},
    ],
    anabutti: [
